...
int pack(InstData data){
    int val = 0;
    val = data.opcode;
    if (data.type != S && data.type != B){
        val |= ((data.rd & 0x1F) << 7);
    }
    if (data.type != U && data.type != J){
        val |= ((data.funct3 & 0x7) << 12);
        val |= ((data.rs1 & 0x1F) << 15);
        if (data.type != I){
            val |= ((data.rs2 & 0x1F) << 20);
        }
    }
    if (data.type == R){
        val |= ((data.funct7 & 0x7F) << 25);
    }
    if (data.type == I){
        val |= ((data.imm & 0xFFF) << 20);
    }
    if (data.type == S){
        val |= ((data.imm & 0x1F) << 7);
        val |= ((data.imm & 0xFE0) << 20);
    }
    if (data.type == B){
        val |= ((data.imm >> 11) & 0x1) << 7;  
        val |= ((data.imm >> 1) & 0xF) << 8; 
        val |= ((data.imm >> 5) & 0x3F) << 25;  
        val |= ((data.imm >> 12) & 0x1) << 31; 
    }
    if (data.type == U){
        val |= ((data.imm & 0xFFFFF) << 12);
    }
    if (data.type == J){
        val |= (data.imm & 0xFF000);
        val |= ((data.imm & 0x800) << 9);
        val |= ((data.imm & 0x7FE) << 20);
        val |= ((data.imm & 0x100000) << 11);
    }
    return val;
}
...
int main()
{ 
    int n = read(0, inbuf, 40);
    InstData data;
    get_inst_data(inbuf, &data);
    int val = pack(data);
    hex_code(val);
    return 0;
}
